...
const ActivityUpdateCard = () => {
  ...
  return <div className="bg-card rounded-lg shadow-sm border border-border overflow-hidden">
      {/* Header */}
      ...
      <div className="px-5 py-5 space-y-5">
        {/* New Activity Form */}
        {showForm && <div className="bg-secondary rounded-lg p-5 space-y-4 border border-border">
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input id="title" placeholder="e.g., Football Training Session" value={activityTitle} onChange={e => setActivityTitle(e.target.value)} className="h-12 rounded-lg border-border bg-card" />
              </div>

              <div>
                <Label htmlFor="description" className="text-sm font-bold text-foreground mb-2 block">Description</Label>
                <Textarea id="description" placeholder="Describe the activities and achievements..." value={activityDescription} onChange={e => setActivityDescription(e.target.value)} rows={3} className="rounded-lg resize-none border-border bg-card" />
              </div>
              ...
              {selectedPhotos.length > 0 && <div className="grid grid-cols-3 gap-3">
                  {selectedPhotos.map((photo, index) => <div key={index} className="relative group">
                      <div className="aspect-square bg-secondary rounded-lg flex items-center justify-center border border-border">
                        <ImageIcon className="h-8 w-8 text-muted-foreground" />
                      </div>
                      <button onClick={() => removePhoto(index)} className="absolute -top-2 -right-2 bg-destructive text-destructive-foreground rounded-full p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <X className="h-4 w-4" />
                      </button>
                    </div>)}
                </div>}
              ...
              <div className="flex gap-3 pt-2">
                <Button onClick={handleSubmitActivity} disabled={isSubmitting} className="flex-1 h-12 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-bold">
                  {isSubmitting ? "Sharing..." : "Share Activity"}
                </Button>
                <Button onClick={() => setShowForm(false)} variant="outline" className="h-12 px-6 rounded-lg border-border">
                  Cancel
                </Button>
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          {recentActivities.map(activity => <div key={activity.id} className="bg-secondary rounded-lg p-4 border border-border">
